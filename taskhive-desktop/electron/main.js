...
async function readRendererSessionUser() {
  try {
    return await runInRenderer(`(() => {
      try {
        return JSON.parse(localStorage.getItem('taskhive_user') || 'null');
      } catch (_error) {
        return null;
      }
    })()`)
...
function startDesktopTrackingTimer() {
  timerRunning = true
...
function stopDesktopTrackingTimer() {
  timerRunning = false
  timerStartedAt = null
  elapsedSeconds = 0
  lastApp = null
...
async function appendRendererTrackingLog(entry) {
  try {
    await runInRenderer(`(() => {
      const key = 'taskhive_tracking_logs';
      const items = JSON.parse(localStorage.getItem(key) || '[]');
      items.unshift(${JSON.stringify(entry)});
      localStorage.setItem(key, JSON.stringify(items.slice(0, 5000)));
      ${broadcastRendererEvent('taskhive:data-sync')}
      return true;
    })()`)
  } catch (_error) {
...
async function syncTrackingLogsToRenderer() {
  try {
    const logs = getAllLogs().map((log) => ({
      id: log.id,
      email: log.email || '',
      appName: log.appName,
      windowTitle: log.windowTitle || log.website || log.appName,
      website: log.website,
    ...
    await runInRenderer(`(() => {
      localStorage.setItem('taskhive_tracking_logs', JSON.stringify(${JSON.stringify(logs)}));
      ${broadcastRendererEvent('taskhive:data-sync')}
      return true;
    })()`)
  } catch (_error) {
...
async function persistDesktopTimeEntry(startMs, endMs) {
  ...
  await appendRendererTimeEntry({
    ...
    description: `${lastApp || 'Desktop'}${lastWebsite ? ` â€¢ ${lastWebsite}` : ''}`,
...
async function trackActivity() {
  ...
  if (isBrowser) {
    const cleanTitle = title
      .replace(/ - Microsoft Edge| Ã¢â‚¬â€œ Microsoft Edge| - Google Chrome| Ã¢â‚¬â€œ Google Chrome| - Personal| Ã¢â‚¬â€œ Personal/i, '')
      .trim()
    ...
    if (lower.includes('outlook') || lower.includes('office.com')) {
      ...
      website = 'google.com'
    } else if (cleanTitle.includes('.') && !cleanTitle.includes('Ã¢â‚¬â€œ')) {
      website = cleanTitle
    }
  }
  ...
  const startTime = endTime - TRACKING_INTERVAL_MS
  ...
  await appendRendererTrackingLog({
    id: `log-${endTime}`,
    userId: currentUser.id,
    userName: currentUser.name,
    ...
    duration: TRACKING_INTERVAL_MS / 1000
  })
  await syncTrackingLogsToRenderer()

  if (appChanged || siteChanged) {
    await flushCurrentActivity(endTime)
    lastApp = appName
    lastWebsite = website
    lastStart = endTime
  } else {
...
app.whenReady().then(() => {
  ...
  setTimeout(async () => {
    await syncTrackingLogsToRenderer()
    const storedSession = getDesktopSession() || await readRendererSessionUser()
    ...
    currentUser = {
      ...
      department: storedSession.department,
...
ipcMain.handle('taskhive:get-tracking-logs', (_event, email) => {
  if (email) {
    return getLogsByEmail(email)
  }
